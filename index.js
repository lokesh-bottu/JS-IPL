...
function main() {
	...
	problemsToExecute.forEach((problemNumber) => {
		const outputFilePath = `src/public/output/output${problemNumber}.json`;

		if (problemNumber === 1) {
			const matchesData = csv().fromFile(matchesPath)
			...
			const matchesData = csv().fromFile(matchesPath);
			const deliveriesData = csv().fromFile(deliveriesPath);
